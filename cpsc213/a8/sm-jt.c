...
int exec() {
  ...
  static const void* jumpTable[16] = {
      &&L00, &&L01, &&L02, &&L03, &&L04, &&DEFAULT, &&L06, &&L07, &&L08,
      &&L09, &&LA, &&LB, &&LC, &&LD, &&LE, &&LF,
  };

  static const void* littleJumpTable[10] = {
  ...
  if (insOpCode < 0x0 || insOpCode > 0xF) {
      ...
      L00://case 0x0: // ld $i, d .............. 0d-- iiii iiii
        reg [insOp0] = insOpExt;
        goto CONT;
      L01://case 0x1: // ld o(rs), rd .......... 1osd
        addr = (insOp0 << 2) + reg [insOp1];
        reg [insOp2] = mem [addr] << 24 | mem [addr+1] << 16 | mem [addr+2] << 8 | mem [addr+3];
        goto CONT;
      L02://case 0x2: // ld (rs, ri, 2), rd .... 2sid
        addr = reg [insOp0] + (reg [insOp1] << 2);
        reg [insOp2] = mem [addr] << 24 | mem [addr+1] << 16 | mem [addr+2] << 8 | mem [addr+3];
        goto CONT;
      L03://case 0x3: // st rs, o(rd) .......... 3sod
        ...
        mem [addr+3] = val       & 0xff;
        goto CONT;
      L04://case 0x4: // st rs, (rd, ri, 4) .... 4sdi
      ...
      L06://case 0x6: // ALU ................... 6-sd
        ...
        if (insOp0 < 0 || insOp0 > 7) {
            ...
            goto *littleJumpTable[insOp0];
        }

        T00://case 0x0: // mov rs, rd ........ 60sd
          reg [insOp2] = reg [insOp1];
              goto CONT;
        T01://case 0x1: // add rs, rd ........ 61sd
          reg [insOp2] = reg [insOp1] + reg [insOp2];
          goto CONT;
        T02://case 0x2: // and rs, rd ........ 62sd
          reg [insOp2] = reg [insOp1] & reg [insOp2];
          goto CONT;
        T03://case 0x3: // inc rr ............ 63-r
          reg [insOp2] = reg [insOp2] + 1;
          goto CONT;
        T04://case 0x4: // inca rr ........... 64-r
          reg [insOp2] = reg [insOp2] + 4;
          goto CONT;
        T05://case 0x5: // dec rr ............ 65-r
          reg [insOp2] = reg [insOp2] - 1;
          goto CONT;
        T06://case 0x6: // deca rr ........... 66-r
          reg [insOp2] = reg [insOp2] - 4;
          goto CONT;
        T07://case 0x7: // not ............... 67-r
          reg [insOp2] = ~ reg [insOp2];
          goto CONT;
        TF://case 0xf: // gpc ............... 6f-r
      ...
      L07://case 0x7: // sh? $i,rd ............. 7dii
        if (insOpImm > 0)
          reg [insOp0] = reg [insOp0] << insOpImm;
        else
          reg [insOp0] = reg [insOp0] >> -insOpImm;
        goto CONT;
      L08://case 0x8: // br o .................. 8-oo
        pc += insOpImm << 1;
        goto CONT;
      L09://case 0x9: // beq rs, o ............. 9roo
        if (reg [insOp0] == 0)
          pc += insOpImm << 1;
        goto CONT;
      LA://case 0xa: // bgt rs, o .............. aroo
        if (reg [insOp0] > 0)
          pc += insOpImm << 1;
        goto CONT;
      LB://case 0xb: // j i ................... b--- iiii iiii
        pc = insOpExt;
        goto CONT;
      LC://case 0xc: // j o(rr) ............... croo
        pc = (((unsigned short) insOpImm) << 1) + reg [insOp0];
        goto CONT;
      LD://case 0xd: // j *o(rr) .............. droo
        addr = ((((unsigned short) insOpImm) << 2) + reg[insOp0]);
        pc = mem [addr] << 24 | mem [addr+1] << 16 | mem [addr+2] << 8 | mem [addr+3];
        goto CONT;
      LE://case 0xe: // j*(rr,ri,4) ............. eri-
        addr = reg[insOp0] + (reg[insOp1] << 2);
        pc = mem [addr] << 24 | mem [addr+1] << 16 | mem [addr+2] << 8 | mem [addr+3];
        goto CONT;
      LF://case 0xf: // halt or nop ............. f?--
        if (insOp0 == 0)
          cont = 0;
        goto CONT;
      DEFAULT://default:
