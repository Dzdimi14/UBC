...
### COME UP WITH ONE FOR APARTMENT PLANNING!!
...
#def nextto():
def isclose(pos,x):
    #p1 and p2 are both coords x,y
    def close(x):
        return abs(pos[0] - x[0]) == 1 or abs(pos[1] - x[1]) == 1
    close.__name__ = str(pos1)+"close"
    return close
...
development = CSP({'HC': plots, 'BH':plots, 'GD':plots, 'RA': plots}, #Housing Complex, Big Hotel, Garbage Dump, Rec Area
                  [
                      #cant be on the same plot
                      Constraint(['HC', 'BH'], ne), Constraint(['HC', 'GD'], ne), Constraint(['HC', 'RA'], ne), 
                      Constraint(['BH', 'HC'], ne), Constraint(['BH', 'GD'], ne), Constraint(['BH', 'RA'], ne),
                      Constraint(['GD', 'BH'], ne), Constraint(['GD', 'HC'], ne), Constraint(['GD', 'RA'], ne),
                      Constraint(['RA', 'BH'], ne), Constraint(['RA', 'GD'], ne), Constraint(['RA', 'GD'], ne),

                      #cant be in cemetery (0,0)

                   ###   
                     Constraint(['HC'], ne_((0,0))), Constraint(['BH'], ne_((0,0))),
                     Constraint(['GD'], ne_((0,0))), Constraint(['RA'], ne_((0,0))),
                     #cant be in lake (1,2)
                     Constraint(['HC'], ne_((1,2))), Constraint(['BH'], ne_((1,2))),
                     Constraint(['GD'], ne_((1,2))), Constraint(['RA'], ne_((1,2))),


                     #HC and BH cant be next to cemetary

                     Constraint(['HC'], notclose((0,0))),
                     #Constraint(['BH',(0,0)], notclose),
                      
                     # rec area should be close to lake
                     Constraint(['RA'], isclose((1,2))),


                     #housing complex and big hotel shoudl be close to rec area
                     Constraint(['HC', 'RA'], isclose),
                     Constraint(['BH', 'RA'], isclose),
                     
                     #housing complex and big hotel should not be close to the garbage dump
                     Constraint(['HC', 'GD'], notclose),
                     Constraint(['BH', 'GD'], notclose)
