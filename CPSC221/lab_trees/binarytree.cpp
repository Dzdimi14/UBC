...
template <typename T>
void BinaryTree<T>::printPaths(const Node* subRoot, int t, int s[]) const {
    ...
    s[temp] = subRoot->elem;
    temp++;
    printPaths(subRoot->left, temp, s);
    ...
    printPaths(subRoot->right, temp, s);
    //temp--;
}
...
int BinaryTree<T>::sumDistances(const Node* subRoot, int count) const{
    ...
    if (subRoot->left == NULL) {
        return count + sumDistances(subRoot->right, count + 1);
    }
    if (subRoot->right == NULL) {
        return count + sumDistances(subRoot->left, count + 1);
    } else {
        return count + sumDistances(subRoot->left, count + 1)
                     + sumDistances(subRoot->right, count + 1);
